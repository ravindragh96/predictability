...
TEST_PATH = os.path.join(BASE_DIR, "synthetic_eta_98.xlsx")
        ...
        with c1:
            fig_mape = go.Figure(data=[
                go.Pie(labels=['MAPE (%)', 'Accuracy (%)'],
                       ...
                       hole=0.6, marker_colors=['#EF553B', "#088262"],
                       textinfo='label+percent')
            ])
            fig_mape.update_layout(title=dict(text=f"MAPE: {global_mape:.2f}%", x=0.1),
        ...
        with c2:
            fig_avg = go.Figure(data=[
                go.Pie(labels=['Avg Error (%)', 'Accuracy (%)'],
                       ...
                       hole=0.6, marker_colors=["#E96847", "#4B0FC2"],
                       textinfo='label+percent')
            ])
            fig_avg.update_layout(title=dict(text=f"Avg Error: {avg_error:.2f}%", x=0.1),
                                  showlegend=False, height=250)
        ...
        with c3:
            fig_local = go.Figure(data=[
                go.Pie(labels=['Local MAPE (%)', 'Accuracy (%)'],
                       ...
                       hole=0.6, marker_colors=["#18AD1D", "#F32819"],
                       textinfo='label+percent')
            ])
            fig_local.update_layout(title=dict(text=f"Local MAPE: {local_mape:.2f}%", x=0.1),
                                    showlegend=False, height=250)
