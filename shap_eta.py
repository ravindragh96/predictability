...
# Force H1 constant = 100 (for all records)
...
if not feature_x or not feature_y or feature_x == feature_y:
    st.warning("Please select two distinct features to visualize.")
    st.stop()
...
# Align and scale test data
X_test = X_test.reindex(columns=all_features, fill_value=X_mean.mean())
X_test_scaled = x_scaler.transform(X_test.astype(np.float32))

# Predict test set for MAPE verification
y_pred_scaled = model.predict(X_test_scaled, verbose=0)
y_pred = y_scaler.inverse_transform(y_pred_scaled)

if output_to_plot in y_actual_df.columns:
    y_actual = y_actual_df[output_to_plot].values
    mape_val = mean_absolute_percentage_error(y_actual, y_pred[:, output_index]) * 100
    st.success(f"✅ Verified MAPE for {output_to_plot}: {mape_val:.2f}% (Expected ≈ 3.33%)")
else:
    y_actual = np.zeros_like(y_pred[:, output_index])
    st.warning(f"⚠️ Actual values for {output_to_plot} not found in test data.")

# -----------------------
# 4️⃣ Build Contour Grid
# -----------------------
...
fig = go.Figure(data=go.Contour(
    ...
    colorscale='Viridis',
    colorbar=dict(
        title=f"{output_to_plot} (Actual Scale)",
        titleside='right'
    ),
