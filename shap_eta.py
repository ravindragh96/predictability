...
st.title("🎛️ Response Surface Modeling (RSM) — Global vs Local Error Visualization")
...
# -----------------------
# 6️⃣ Predict Test Data (Global metrics)
# -----------------------
...
# -----------------------
st.set_page_config(page_title="RSM Contour App", layout="wide")
st.title("🎛️ Response Surface Modeling (RSM) — Global & Local Error Visualization")
...
# -----------------------
# 8️⃣ Calculate Local MAPE (for visible feature range)
# -----------------------
mask = (
    (X_test[f1] >= X_test[f1].min()) & (X_test[f1] <= X_test[f1].max()) &
    (X_test[f2] >= X_test[f2].min()) & (X_test[f2] <= X_test[f2].max())
)
local_errors = percent_errors[mask]
local_mape = np.mean(local_errors) if len(local_errors) > 0 else np.nan

# -----------------------
# 9️⃣ Split Layout: Contour + Donut Charts
# -----------------------
col1, col2 = st.columns([2, 1])
    ...
    if output_to_plot in y_actual_df.columns:
        ...
        with c1:
            ...
            fig_mape.update_layout(
                title=dict(text=f"Global MAPE: {global_mape:.2f}%", x=0.5),
                showlegend=False,
                height=250
            )
            st.plotly_chart(fig_mape, use_container_width=True)
        ...
        with c3:
            fig_local = go.Figure(data=[
                go.Pie(
                    labels=['Local MAPE (%)', 'Accuracy (%)'],
                    values=[local_mape if not np.isnan(local_mape) else 0, 100 - local_mape if not np.isnan(local_mape) else 100],
                    hole=0.6,
            ...
            fig_local.update_layout(
                title=dict(text=f"Local MAPE: {local_mape:.2f}%", x=0.5),
                showlegend=False,
                height=250
            )
            st.plotly_chart(fig_local, use_container_width=True)
